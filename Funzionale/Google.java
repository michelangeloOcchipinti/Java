...
//Chiedo all'utente d'inserire i dati da cercare e faccio un controllo se è stata inserita solamente una parola inviando nel caso un messaggio d'errore
        ...
        System.out.println("Inserisci le due parole da cercare in un'unica riga separate da uno spazio e seguite da un invio: ");
        System.out.println();
        String fraseInserita=input.nextLine();
        if (!fraseInserita.contains(" ")){
			System.out.println();
			System.out.println("Hai inserito solamente una parola, ricomincia dall'inizio!");
			System.out.println();
										}
        else{
...
//Se verrà trovata la prima parola si aumenterà il valore di k ad  uno così da cercare la seconda parola
							...
							k=1;
							if (j==frasi[0].length-1 || j==frasi[0].length-2) j=j-1;
							if (fraseDaCercare[k].equals(frasi[i][j+1])){
